...
class DLL:
    ...
    def size(self):
        """
        Traverses the Linked List and returns an integer value representing the number 
        of nodes in the Linked List.
        
        The time complexity is 0(n) because every Node in tje Linked List must be 
    ...
    def search(self, data):
        ...
        The time complexity is 0(n) becaus in the worst
        """
        if self.head is None:
    ...
    def remove(self, data):
        """Remove the first occurence of a Node that contains the data argument
        ...
        while not found:
            if current.get_data() == data:
                ...
                if current.get_next() is None:
                    return "A node with that data values is not present."
                else:
                    current = current.get_next()
